...
{
    public partial class MidTermPreview
    {
        ...
        async Task OnClassChanged(IEnumerable<string> e)
        {
            ...
            await RunMidTermScoresPreview();
        }

        #endregion

        #region [Scores Preview]        
        async Task RunMidTermScoresPreview()
        {
            ...
            i = 0;
            j = 0;
            progressbarInfo = "Please wait loading Mid-Term Scores...";
            ...
            foreach (var item in cognitiveMarks)
            {
                ...
                if ((item.Mark_MidCBT + item.Mark_Mid) > 0)
                {
